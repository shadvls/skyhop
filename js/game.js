...
function drawPipes(){for(var i=0;i<pipes.length;i++){var p=pipes[i];ctx.fillStyle=PIPE_BODY;ctx.fillRect(p.x,0,pipeW,p.gapY-pipeGap/2);ctx.fillRect(p.x,p.gapY+pipeGap/2,pipeW,groundY-p.gapY-pipeGap/2);ctx.fillStyle=PIPE_CAP;ctx.fillRect(p.x-4,p.gapY-pipeGap/2-15,pipeW+8,15);ctx.fillRect(p.x-4,p.gapY+pipeGap/2,pipeW+8,15)}}
...
function applyShake(){if(shakeTimer>0)shakeTimer--}
...
function handleSkinClick(x,y){var names=["yellow","red","blue","gold","raven"];for(var i=0;i<names.length;i++){var cx=W/2-60+i*30,cy=410;if(Math.abs(x-cx)<12&&Math.abs(y-cy)<12){setSkin(names[i]);return true}}return false}
...
function handleDiffClick(x,y){for(var i=0;i<4;i++){var cx=W/2-60+i*40,cy=390;if(Math.abs(x-cx)<20&&Math.abs(y-cy)<10){difficulty=i;applyDifficulty(difficulty);try{localStorage.setItem("skyhop_diff",difficulty)}catch(e){};return true}}return false}
function handleThemeClick(x,y){var names=["default","sunset","midnight","arctic","swamp"];for(var i=0;i<names.length;i++){var rx=W/2-80+i*32,ry=440;if(x>=rx&&x<=rx+28&&y>=ry&&y<=ry+10){setTheme(names[i]);return true}}return false}
...
function getPos(e){var t=e.touches&&e.touches[0]||e.changedTouches&&e.changedTouches[0];var cx=e.clientX||(t&&t.clientX)||0,cy=e.clientY||(t&&t.clientY)||0;var r=c.getBoundingClientRect();return{x:(cx-r.left)/scale,y:(cy-r.top)/scale}}
...
function onTap(e){var pos=getPos(e);initAudio();if(restartDelay>0)return;if(state==S.MENU){if(handleSkinClick(pos.x,pos.y)||handleDiffClick(pos.x,pos.y)||handleThemeClick(pos.x,pos.y))return;resetGame();birdFlap()}else if(state==S.PLAYING){if(!stats.games)unlockAchievement("firstFlap");birdFlap();playFlap();emitBurst(birdX,birdY,3,"rgba(255,255,255,0.5)")}else if(state==S.GAMEOVER&&restartDelay<=0){goToMenu()}}
